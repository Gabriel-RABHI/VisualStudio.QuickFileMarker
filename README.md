...
## Purpose
QuickFileMarker is a lightweight, cross-platform-ready developer extension that allows users to rapidly generate contextual "marker" files directly from their IDE's text editor. This current version is a Visual Studio 2022 extension.
...
## Usage & Configuration
QuickFileMarker generates a portable configuration file `extention-config.json` inside your system's Application Data folder (`~/.config/QuickFileMarker` or `%APPDATA%\QuickFileMarker`). 

By modifying this JSON, you can dynamically control the Context Menu entries and assign custom flags and behaviors without recompiling the extension.
